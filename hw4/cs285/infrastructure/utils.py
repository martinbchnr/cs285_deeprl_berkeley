...
def calculate_mean_prediction_error(env, action_sequence, models, data_statistics):
    ...
    mpe = np.mean(np.power(true_states - pred_states,2))  # (Q1)
...
def sample_trajectory(env, policy, max_path_length, render=False, render_mode=('rgb_array')):
    ...
    while True:
        ...
        obs.append(ob)
        ac = policy.get_action(ob) # from HW1
        ac = ac[0]
        acs.append(ac)
        ...
        rollout_done = (steps == max_path_length) or done # from HW1
