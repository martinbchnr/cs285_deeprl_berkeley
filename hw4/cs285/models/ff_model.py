...
class FFModel(BaseModel):
    ...
    def define_forward_pass(self):
        ...
        # (Q1) Define obs_normalized using obs_unnormalized,and self.obs_mean_pl and self.obs_std_pl   
        obs_normalized = normalize(obs_unnormalized, self.obs_mean_pl, self.obs_std_pl) 
        
        # (Q2) Define acs_normalized using acs_unnormalized and self.acs_mean_pl and self.acs_std_pl
        ...
        self.delta_pred_normalized = build_mlp(concatenated_input, self.ob_dim, self.scope, self.n_layers,self.size)  # (Q1) Use the build_mlp function and the concatenated_input above to define a neural network that predicts unnormalized delta states (i.e. change in state)
        
        
        self.delta_pred_unnormalized = unnormalize(self.delta_pred_normalized, self.obs_mean_pl, self.obs_std_pl) # (Q1) Unnormalize the the delta_pred above using the unnormalize function, and self.delta_mean_pl and self.delta_std_pl
        
        
        self.next_obs_pred = self.delta_pred_unnormalized + self.obs_pl  # (Q1) Predict next observation using current observation and delta prediction (not that next_obs here is unnormalized)
